...
| X02 | Residual / offsets claim depth         | `/compliance/residual`         | Done\* — project/methodology fields, claim disclosure guard, `?pack=1` claim pack download                                               | 2    | M      |
| X03 | Restatements → applied inventory       | `/compliance/ghg/restatements` | Done\* — `appliedAt`, `getAppliedBaseYearInventory`, `/restatements/as-of` panel                                                         | 2    | L      |
| X04 | Double materiality depth               | `/materiality`                 | Done\* — ESRS crosswalk chips, IRO register per topic, stakeholder survey avg + notes                                                    | 1    | L      |
| X05 | BRSR depth (India)                     | `/frameworks/brsr`             | Done\* — `buildBrsrPack` wired on coverage `?pack=1` + download button                                                                   | 1    | M      |
| X06 | Assurance L vs R engagement depth      | `lib/assurance/pathways`       | Done\* — materiality/sampling fields, checkpoint `evidenceIds`, opinion draft download                                                   | 1    | M      |
| X07 | Multi-org consolidation depth          | Org hierarchy + consolidation  | Done\* — `applyIcEliminations`, statutory (full-method) vs management pack toggle                                                        | 3    | L      |
| X08 | Employee engagement depth              | `/engagement`                  | Done\* — public tokenised commute survey (`/e/[token]`), response counter, survey link                                                   | 3    | M      |
| X09 | Procurement tradeoffs depth            | `/procurement/tradeoffs`       | Done\* — `buildRfpPack` (RFQ + weights + ranked options + Pareto + gaps), `?pack=1`, download buttons                                    | 3    | M      |
| X10 | Cascade ↔ MACC ↔ reduction closed loop | analytics routes               | Done\* — `abatementLevers`/`reductionProjects` links, `getDecarbonPlan`, `/analytics/decarbon-plan`, panel + MACC/projects links         | 2    | M      |
| X11 | Public embed hardening                 | `/public/reports/embed`        | Done\* — allowlist + theme on `ReportEmbedTokens`, origin enforcement, dynamic `frame-ancestors` CSP via `proxy.ts`, `docs/embed-csp.md` | 3    | S      |
| X12 | Realtime SSE multi-instance            | `lib/realtime`                 | Done\* — `bus.ts` Upstash RPUSH/poll fan-out with local-hub fallback, wired into `emit.ts` + SSE route                                   | 3    | M      |
| X13 | California SB 253/261 filing pack      | `/compliance/california`       | Done\* — `buildCaliforniaPack` wired on coverage `?pack=1` + download button                                                             | 2    | M      |
| X14 | Taxonomy / ISSB mapping stubs          | `mappings.ts`, ISSB questions  | Done\* — real ISSB S1/S2 GHG/workforce/governance + EU Taxonomy eligibility mappings, `s2-ghg` `metricKeys`                              | 2    | M      |
