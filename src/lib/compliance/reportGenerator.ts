...
import { generateDataQualityNarrative, getDataQualityRating } from "./dataQualityAssessor";
import { generateBoundaryNarrative } from "./boundaryValidator";
...
export async function generateComplianceReport(
  ...
  const verified = (checkpoints.docs || []).filter(cp => cp.status === "verified").length;
  ...
  const narrative = generateNarrative({
    ...
    verifiedBy: compliance.verifiedBy?.id,
...
function generateNarrative(data: ComplianceReportData): string {
  ...
  parts.push(`This report documents emissions calculations, data quality, and compliance with international GHG accounting standards.\n`);
  ...
  parts.push(`| Scope 1 (Direct) | ${data.scope1Total.toLocaleString()} | ${scope1Pct}% |`);
  parts.push(`| Scope 2 (Indirect - Energy) | ${data.scope2Total.toLocaleString()} | ${scope2Pct}% |`);
  parts.push(`| Scope 3 (Other Indirect) | ${data.scope3Total.toLocaleString()} | ${scope3Pct}% |`);
  ...
  parts.push(
    `**Checkpoints Fulfilled:** ${data.checkpointsFulfilled}/${data.checkpointsTotal} (${
      total > 0 ? ((data.checkpointsFulfilled / data.checkpointsTotal) * 100).toFixed(1) : "0.0"
    }%)\n`
  );

  if (data.complianceScore >= 90) {
    parts.push("**Status:** ✓ **EXCELLENT** - Organization is highly compliant with GHG Protocol standards.\n");
  } else if (data.complianceScore >= 75) {
    parts.push("**Status:** ✓ **GOOD** - Organization meets most GHG Protocol requirements.\n");
  } else if (data.complianceScore >= 60) {
    parts.push("**Status:** ⚠ **ACCEPTABLE** - Organization meets baseline requirements but has improvement opportunities.\n");
  } else {
    parts.push("**Status:** ✗ **NEEDS IMPROVEMENT** - Organization should address identified gaps before assurance.\n");
  }
  ...
  if (data.dataQualityScore >= 80) {
    parts.push("Data quality is **high** and suitable for third-party assurance audit.\n");
  ...
  parts.push(`*This report is audit-ready and suitable for third-party GHG Protocol verification.*\n`);
