...
import {
  computeFreight,
  FREIGHT_DOWNSTREAM_MODES,
  FREIGHT_METRIC_KEYS,
  FREIGHT_UPSTREAM_MODES,
  type DatapointValue,
} from "@/lib/calc";
...
const FREIGHT_METRIC_SET = new Set<string>(FREIGHT_METRIC_KEYS);

const FIELD_META = [
  ...FREIGHT_UPSTREAM_MODES.map((m) => ({
    metricKey: m.metricKey,
    label: m.label,
    unit: "tkm",
    group: "upstream" as const,
  })),
  ...FREIGHT_DOWNSTREAM_MODES.map((m) => ({
    metricKey: m.metricKey,
    label: m.label,
    unit: "tkm",
    group: "downstream" as const,
  })),
];
...
function parseActivityBody(body: unknown):
  ...
  for (const row of record.activities) {
    ...
    if (!FREIGHT_METRIC_SET.has(metricKey)) {
...
/**
 * GET /api/app/scope3/freight?periodId=
 * Membership-gated read of Cat 4/9 freight datapoints + computed tCO2e preview.
 */
  ...
  try {
    ...
    if (periods.docs.length === 0) {
      return NextResponse.json({
        ...
        computation: null,
        message:
          "No reporting period. Create one under Metrics before entering freight data.",
      });
    }
    ...
    const dps = await payload.find({
      ...
      where: {
        and: [
          ...
          { metricKey: { in: [...FREIGHT_METRIC_KEYS] } },
        ],
      },
      limit: 100,
      overrideAccess: true,
    ...
    let computation: ReturnType<typeof computeFreight> | null = null;
    let computeError: string | null = null;
    try {
      computation = computeFreight(metrics, factors, region, year);
    } catch (err) {
...
/**
 * POST /api/app/scope3/freight
 * Body: { periodId, activities: [{ metricKey, value }] }
 * Writes Membership-gated datapoints for Cat 4/9 freight metrics.
 */
