...
import { markReadData } from "@/lib/notifications";
import { findNotificationById, updateNotification } from "@/lib/notifications/store";
...
/** PATCH /api/app/notifications/[id]/read — mark one notification read. */
export async function PATCH(_req: Request, routeCtx: Ctx) {
  ...
  if (doc.isRead === true) {
    return NextResponse.json({ ok: true, id: doc.id, isRead: true });
  }

  const updated = await updateNotification(payload, doc.id, markReadData());
