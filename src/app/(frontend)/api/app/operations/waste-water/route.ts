...
import {
  WASTE_WATER_ACTIVITY_FIELDS,
  WASTE_WATER_ALL_METRIC_KEYS,
  WASTE_WATER_METRIC_KEYS,
  computeWasteCat5,
  wasteDiversionRate,
  wasteWaterCoverageGaps,
  waterIntensityPerFte,
  type DatapointValue,
} from "@/lib/calc";
...
const ACTIVITY_METRIC_SET = new Set<string>(
  WASTE_WATER_ACTIVITY_FIELDS.map((f) => f.metricKey),
);

const FIELD_META = WASTE_WATER_ACTIVITY_FIELDS.map((f) => ({
...
function parseActivityBody(body: unknown):
  ...
  for (const row of record.activities) {
    ...
    if (!ACTIVITY_METRIC_SET.has(metricKey)) {
...
/**
 * GET /api/app/operations/waste-water?periodId=
 * Membership-gated read of operational waste/water datapoints + optional Cat 5 preview.
 */
  ...
  try {
    ...
    if (periods.docs.length === 0) {
      return NextResponse.json({
        ...
        activities: [],
        coverageGaps: [],
        wasteComputation: null,
        waterIntensity: null,
        diversionRate: null,
        message:
          "No reporting period. Create one under Metrics before entering waste or water data.",
      });
    }
    ...
    const dps = await payload.find({
      ...
      where: {
        and: [
          ...
          { metricKey: { in: [...WASTE_WATER_ALL_METRIC_KEYS] } },
        ],
      },
      limit: 100,
      overrideAccess: true,
    ...
    const employeesRow = byKey.get(WASTE_WATER_METRIC_KEYS.employeesTotal);
    if (employeesRow && employeesRow.value !== null) {
      metrics[WASTE_WATER_METRIC_KEYS.employeesTotal] = {
        value: employeesRow.value,
        quality: employeesRow.quality === "estimated" ? "estimated" : "measured",
      };
    }
    ...
    const wasteComputation = computeWasteCat5(metrics, factors, region, year);
    const withdrawal = metrics[WASTE_WATER_METRIC_KEYS.waterWithdrawal]?.value ?? null;
    const employees = metrics[WASTE_WATER_METRIC_KEYS.employeesTotal]?.value ?? null;
    const waterIntensity = waterIntensityPerFte(withdrawal, employees);
    const diversionRate = wasteDiversionRate(metrics);
    const coverageGaps = wasteWaterCoverageGaps(activities);
    ...
    return NextResponse.json({
      ...
      year,
      standard,
      employeesFte: employees,
      waterIntensity,
      diversionRate,
      wasteComputation: {
        totalTco2e: wasteComputation.measured.value,
        quality: wasteComputation.measured.quality,
        emissionsComputed: wasteComputation.emissionsComputed,
        components: wasteComputation.components.map((c) => ({
          key: c.key,
          label: c.label,
        ...
        missingInputs: wasteComputation.missingInputs,
        missingFactors: wasteComputation.missingFactors,
      },
    });
  } catch (error) {
...
/**
 * POST /api/app/operations/waste-water
 * Body: { periodId, activities: [{ metricKey, value }] }
 * Writes Membership-gated datapoints for operational waste/water metrics.
 */
  ...
  try {
    ...
    for (const activity of parsed.activities) {
      const result = await writeDatapoint(payload, {
        ...
        source: "manual",
        actorId: ctx.user.id,
        reason: "Operations waste / water entry",
      });
      written.push({
        metricKey: activity.metricKey,
