...
import { calculateSKUFootprint, updateSKUFootprint } from "@/lib/calc/skuFootprint";
...
export async function POST(request: Request) {
  try {
    ...
    let result;
    if (recalculate) {
      result = await updateSKUFootprint(skuId);
    } else {
      result = await calculateSKUFootprint(skuId);
    }
